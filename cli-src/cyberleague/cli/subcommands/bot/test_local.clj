...
(defn- test-ping-pong!
  ...
  (let [run-cmd (:bot/run-cmd bot-config)
        ...
        ping-id (str (java.util.UUID/randomUUID))
    ...
    (cond
      (= :timeout result)
      (do (println (f/color :color/red "  FAIL") "timed out after 10 seconds") false)
      ...
      (try
        (let [response (json/parse-string (:out result) true)]
          ...
          (if (= (:pong response) ping-id)
            (do (println (f/color :color/green "  PASS")) true)
            (do (println (f/color :color/red "  FAIL") "expected {:pong" (str "\"" ping-id "\"") "}") false)))
        (catch Exception _
...
(defn- test-context!
  ...
  (let [run-cmd (:bot/run-cmd bot-config)
        ...
        game-id (->> games
                     ...
                     :game/id)]
    (if-not game-id
      (do (println (f/color :color/red "  ERROR") "game not found:" game-slug) false)
...
(defn test-local!
  ...
  (println "  run-cmd:" (:bot/run-cmd bot-config))
  (let [step1-ok? (test-ping-pong! bot-config)
        step2-ok? (test-context! bot-config)]
    (and step1-ok? step2-ok?)))
