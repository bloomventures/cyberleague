...
(defn eval-move
  ...
  (-> state
      json/write-str
      (evaluator.api/native-code-runner
       (get-in bot [:bot/code :code/env :env/slug])
       (get-in bot [:bot/code :code/code]))
      (update :eval/return-value json/read-str :key-fn keyword)))

(defn run-move [bot state game-engine]
  (let [eval (try
               (eval-move bot (game-engine.protocol/anonymize-state-for game-engine (:bot/id bot) state))
               (catch Exception e
    ...
    (cond
      ...
      {:eval/error {:move.error/type :move.error.type/invalid-move
                    :move.error/data {:bot-id (:bot/id bot)
                                      :move eval}}}

      (not (game-engine.protocol/legal-move? game-engine state (:bot/id bot) return-value))
...
(defn run-game [game bots]
  (let [game-engine (game-engine.protocol/make-engine game)
    ...
    (loop [states [(game-engine.protocol/init-state game-engine (map :bot/id bots))]
           std-outs [""]
           players (cycle bots)]
            ...
            (let [bot (first players)
              ...
              (if (:eval/error eval)
                {:game.result/error (:eval/error eval)
                 ...
                 :game.result/history (:history state)}
                (recur (conj states
                             (game-engine.protocol/next-state game-engine state {(:bot/id bot) (:eval/return-value eval)}))
